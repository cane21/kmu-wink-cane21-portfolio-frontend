...
window.addEventListener('load', async function()
{
    ...
    for(var i = 0 ; i< 7 ; i++)
    {
        ...
        var temp= weatherResponse.daily[i].uvi;

        if(temp<5)
        {
            emotionImg[i].style.background="url(\"../assets/images/smile.png\") no-repeat 50% 50%/100% 100%";
        }
        else
        {
            emotionImg[i].style.background="url(\"../assets/images/bad.png\") no-repeat 50% 50%/75% 75%";
        }
    ...
    for(var i=0 ; i< 7; i++)
    {
        ...
        t[i].innerHTML = weatherResponse.daily[i].uvi;

        // var temp= parseInt(weatherResponse.daily[i].uvi);
        var temp= weatherResponse.daily[i].uvi;

        // console.log(temp);
        if(temp<2)
        {
            windArray[i].innerHTML = "낮음";
    ...
    if(weatherResponse.daily[0].uvi<2)
    {
    ...
    else if(weatherResponse.daily[0].uvi<5)
    {
